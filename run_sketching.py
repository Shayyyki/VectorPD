...
parser.add_argument("--mask_file", type=str,
                    help="mask image file, located in <mask_images>")
...
parser.add_argument("--num_face_paths", type=int, default=20,
                    help="number of face stroke")
parser.add_argument("--num_global_paths", type=int, default=40,
                    help="number of global stroke")
...
parser.add_argument('--gpunum', type=int, default=2)
...
mask = f"{abs_path}/masked_images/{args.mask_file}"
assert os.path.isfile(target), f"{target} does not exists!"
assert os.path.isfile(mask), f"{mask} does not exists!"
...
def run(seed, wandb_name):
    exit_code = sp.run(["python", "rendering_portrait.py", target,mask,
                        "--target_file", str(args.target_file),
                        "--mask_file", str(args.mask_file),
                        "--num_face_paths", str(args.num_face_paths),
                        "--num_global_paths", str(args.num_global_paths),
